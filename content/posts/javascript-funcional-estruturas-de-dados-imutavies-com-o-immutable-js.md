---
title: JavaScript funcional - Estruturas de dados imutavies com o immutable-js
author: Jhonny Roger
...
Pensando nisto, o time do Facebook criou o immutable.js, uma biblioteca que traz estruturas de dados 100% imutáveis para o JavaScript. Ele trabalha com algumas estruturas de dados já conhecidas como List, Map, OrderedMap, Set, OrderedSet, Stack e Record, com a diferença de que todas estas estruturas são implementadas seguindo os conceitos de imutabilidade, 
...
List é uma estrutura de dados com índices numerados, muito semelhante ao que seriam os Arrays em JavaScript puro. Devido as semelhanças, elas são muito utilizadas em situações onde seria necessário fazer o uso de Arrays comuns. Veja alguns exemplos: 
...
Esta é uma estrutura de dados muito semelhante a um Map, com a diferença que ele é ordenado. A ordenação padrão é definida pela ordem de inclusão dos itens, porém existem métodos como o *.sort()* e o *.sortBy()* que podem reordenar os valores do OrderedMap.
...
Basicamente ela funciona como o uma List trabalhando de forma mais limitada, sendo que todas as operações de inclusão e exclusão ocorrerão apenas no topo da pilha. Os métodos básicos serão o *shift* e o *unshift*, porém o *push* e o *pop* neste caso, tem o mesmo efeito e trabalham sempre com o primeiro item da pilha. Veja alguns exemplos: 
...
O Record é uma estrutura para criar esquemas, ou classes com valores padrão que podem ser instanciadas. Com ele por exemplo, é possível definir um valor para um determinado atributo do Record caso ele não seja passado pelo construtor. 
Caso um atributo não esteja no Record e for passado no construtor, este atributo será ignorado.
...
Por enquanto é isso, o immutable é uma ferramenta que pode acrescentar muito em projetos JavaScript que buscam embarcar no paradigma funcional, assegurando que de fato todas as estruturas de dados serão imutáveis. Deixarei uma lista com a documentação da Api de cada uma destas estruturas, com vários exemplos, métodos e recursos mais detalhados sobre o assunto. Até a próxima!
